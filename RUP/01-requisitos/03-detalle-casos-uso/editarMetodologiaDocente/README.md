...
Solo `descripción` es editable -- `código` es el identificador institucional fijo (`MD1`-`MD7`, ver modelo del dominio) y se fija en `crearMetodologiaDocente()`, no se toca después.
...
- Modelo del dominio -- `MetodologiaDocente { codigo, descripcion }`; `MetodologiaMateria.descripcionPropia` es el override por materia, fuera del alcance de este caso (vive en L4)
