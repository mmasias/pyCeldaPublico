...
**Suelo por instrumento** (issue [#298](https://github.com/mmasias/pyCelda/issues/298)): la misma rama roja de la `<<choice>>` cubre ahora dos condiciones -- la `ponderacion` introducida tiene que ser **mayor que cero** (estricto) *y* no pasar de `ponderacionMaxima` del `SistemaEvaluacion` elegido. Una ponderación de `0` o negativa equivale a "el instrumento no existe" y se rechaza con un `422` (`La ponderación de un instrumento debe ser mayor que cero`). El suelo por instrumento es un `> 0` fijo, no dependiente del `SistemaEvaluacion`: `ponderacionMinima` solo acota la **suma** de instrumentos (la valida [`enviarGuiaARevision()`](../enviarGuiaARevision/README.md)), nunca el instrumento individual. `ponderacionMaxima` sí actúa en los dos niveles -- acota el instrumento individual aquí (`validar_maximo`) *y además* la suma en `enviarGuiaARevision()` -- y el techo por instrumento se queda en `<= ponderacionMaxima` (permite el caso real "examen final único = 100 %" con horquilla `[100, 100]`). El `<input>` del formulario lleva `min` y una pista "Mayor que cero"; el mensaje del `422` del techo pasa a decir "del sistema de evaluación" (hallazgo H-10). No es rama nueva del statechart: es otra condición de la `<<choice>>` que ya existía.
...
- [Modelo del dominio](/RUP/00-modelo-del-dominio/modeloDominio.puml) -- `Guia *-d- PonderacionEvaluacion`, `PonderacionEvaluacion -> SistemaEvaluacion`, regla de rango documentada en el README
