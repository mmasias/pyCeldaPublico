...
|![](/images/RUP/01-requisitos/03-detalle-casos-uso/crearGrado/wireframe.svg)|
|-|
...
Primer caso de uso del catálogo que solicita `codigo` en la creación (`Asignatura`/`MetodologiaDocente` también lo tienen, pero sus `crearX()` no lo piden -- divergencia histórica de L0/L1, sin corregir retroactivamente). `codigo` es fijo desde el alta: `editarGrado()` solo permite tocar `nombre`, mismo criterio que `MetodologiaDocente.codigo`.
...
- Modelo del dominio -- `Grado { codigo, estado }`, formalizado en la discussion [#18](https://github.com/mmasias/pyCelda/discussions/18)
